...
func (acw *APICoverageWebhook) registerWebhook(rules []admissionregistrationv1beta1.RuleWithOperations, namespace string) error {
	...
	deployment, err := acw.KubeClient.ExtensionsV1beta1().Deployments(namespace).Get(context.TODO(), acw.DeploymentName, metav1.GetOptions{})
	if err != nil {
	...
	_, err = acw.KubeClient.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations().Create(context.TODO(), webhook, metav1.CreateOptions{})
	if err != nil {
