...
func (client *KubeClient) UpdateConfigMap(name string, configName string, values map[string]string) error {
	configMap, err := client.GetConfigMap(name).Get(context.TODO(), configName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	_, err = client.GetConfigMap(name).Update(context.TODO(), configMap, metav1.UpdateOptions{})
	return err
}
...
func (client *KubeClient) CreatePod(pod *corev1.Pod) (*corev1.Pod, error) {
	...
	return pods.Create(context.TODO(), pod, metav1.CreateOptions{})
}
