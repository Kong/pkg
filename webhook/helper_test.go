...
func createTestConfigMap(t *testing.T, kubeClient kubernetes.Interface) error {
	...
	_, err := kubeClient.CoreV1().ConfigMaps(metav1.NamespaceSystem).Create(context.TODO(), configMaps, metav1.CreateOptions{})
	if err != nil {
		return err
	}
	return nil
...
func createSecureTLSClient(t *testing.T, kubeClient kubernetes.Interface, acOpts *Options) (*http.Client, error) {
	...
	if _, err := kubeClient.CoreV1().Secrets(secret.Namespace).Create(context.TODO(), secret, metav1.CreateOptions{}); err != nil {
		return nil, err
	}
