...
func (r *BaseReconciler) EnsureFinalizer(ctx context.Context, fb kmeta.Accessor) error {
	...
	// ... and apply it.
	_, err = r.DynamicClient.Resource(r.GVR).Namespace(fb.GetNamespace()).Patch(context.TODO(), fb.GetName(),
...
func (r *BaseReconciler) labelNamespace(ctx context.Context, subject tracker.Reference) error {
	...
	_, err = r.DynamicClient.Resource(gvr).Patch(context.TODO(), subject.Namespace, types.MergePatchType, patch, metav1.PatchOptions{})
	if err != nil {
	...
	for _, ps := range referents {
		...
		eg.Go(func() error {
			...
			_, err = r.DynamicClient.Resource(gvr).Namespace(ps.Namespace).Patch(
				context.TODO(), ps.Name, types.JSONPatchType, patchBytes, metav1.PatchOptions{})
			if err != nil {
...
func (r *BaseReconciler) UpdateStatus(ctx context.Context, desired Bindable) error {
	...
	_, err = r.DynamicClient.Resource(r.GVR).Namespace(desired.GetNamespace()).UpdateStatus(
		context.TODO(), forUpdate, metav1.UpdateOptions{})
	return err
}
